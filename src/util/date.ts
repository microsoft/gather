export function getRelativeTime(now: Date, time: Date): string {
    ...
    if (months > 0)
        headerText = relativeTime(months, "month");
    else if (weeks > 0)
        headerText = relativeTime(weeks, "week");
    else if (days > 0)
        headerText = relativeTime(days, "day");
    else if (hours > 0)
        headerText = relativeTime(hours, "hour");
    else if (minutes > 0)
        headerText = relativeTime(minutes, "minute");
    else if (seconds > 0)
        headerText = relativeTime(seconds, "second");
    else
        headerText = "Milliseconds ago";
    return headerText;
}
